...
def get_gabor_filters(size=21, filter_scale=4., num_orients=16, weights=False):
    ...
    def _get_sparse_gaussian():
        """Sparse Gaussian."""
        size1 = 2 * np.ceil(np.sqrt(2.) * filter_scale) + 1
        alt = np.zeros((int(size1), int(size1)), np.float32)
        alt[int(size1 // 2), int(size1 // 2)] = 1
        gaussian1 = gaussian_filter(alt, filter_scale / np.sqrt(2.), mode='constant')
        gaussian1[gaussian1 < 0.05 * gaussian1.max()] = 0
        return gaussian1
...
def generate_suppression_masks(filter_scale=4., num_orients=16):
    ...
    """
    size = 2 * int(np.ceil(filter_scale * np.sqrt(2))) + 1    # np.ceil()向上取整, size默认为13
    cx, cy = size // 2, size // 2    # cx, cy 均默认为 6
    filter_masks = np.zeros((num_orients, size, size), np.float32)    # filter_masks.shape = (16, 6, 6)
    # Compute for orientations [0, pi), then flip for [pi, 2*pi)
    for i, angle in enumerate(np.linspace(0., np.pi, num_orients // 2, endpoint=False)):    # i: 0-7
